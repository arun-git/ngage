...
final groupTeamsProvider =
    StreamProvider.family<List<Team>, String>((ref, groupId) {
...
final memberTeamsProvider =
    StreamProvider.family<List<Team>, String>((ref, memberId) {
  final teamService = ref.watch(teamServiceProvider);
...
final availableMembersProvider =
    FutureProvider.family<List<GroupMember>, String>((ref, teamId) {
  final teamService = ref.watch(teamServiceProvider);
...
final teamsLedByMemberProvider =
    FutureProvider.family<List<Team>, String>((ref, memberId) {
  final teamService = ref.watch(teamServiceProvider);
...
class TeamManagementNotifier extends StateNotifier<AsyncValue<String?>> {
  ...
  Future<void> removeMembersFromTeam(
  ...
  Future<void> changeTeamLead(String teamId, String newTeamLeadId) async {
    ...
    try {
      await _teamService.changeTeamLead(
          teamId: teamId, newTeamLeadId: newTeamLeadId);
  ...
  Future<void> transferTeamLeadership(String teamId, String newTeamLeadId,
      {String? reason}) async {
    state = const AsyncValue.loading();
  ...
  Future<void> updateTeamLogo({
    ...
    try {
      ...
      state = const AsyncValue.data('Team logo updated successfully');
    } catch (error, stackTrace) {
      state = AsyncValue.error(error, stackTrace);
    }
  }
...
final canAddMemberProvider =
    FutureProvider.family<bool, ({String teamId, String memberId})>(
        (ref, params) {
  final teamService = ref.watch(teamServiceProvider);
...
final teamMemberDetailsProvider =
    FutureProvider.family<List<Map<String, dynamic>>, String>((ref, teamId) {
  final teamService = ref.watch(teamServiceProvider);
