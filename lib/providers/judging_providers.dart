...
final judgeScoreProvider =
    FutureProvider.family<Score?, (String, String)>((ref, params) async {
...
final submissionScoresProvider =
    FutureProvider.family<List<Score>, String>((ref, submissionId) async {
  final judgingService = ref.read(judgingServiceProvider);
  return await judgingService.getSubmissionScores(submissionId);
});

/// Provider for getting aggregated scores for a submission
final submissionAggregationProvider =
    FutureProvider.family<AggregatedScore, String>((ref, submissionId) async {
  final judgingService = ref.read(judgingServiceProvider);
...
final eventRubricsProvider =
    FutureProvider.family<List<ScoringRubric>, String>((ref, eventId) async {
  final judgingService = ref.read(judgingServiceProvider);
  return await judgingService.getAvailableRubrics(eventId: eventId);
});

/// Stream provider for real-time event rubrics updates
final eventRubricsStreamProvider =
    StreamProvider.family<List<ScoringRubric>, String>((ref, eventId) {
...
final templateRubricsProvider =
    FutureProvider<List<ScoringRubric>>((ref) async {
  final judgingService = ref.read(judgingServiceProvider);
  return await judgingService.getAvailableRubrics(isTemplate: true);
});

/// Provider for getting group rubrics
final groupRubricsProvider =
    FutureProvider.family<List<ScoringRubric>, String>((ref, groupId) async {
  final judgingService = ref.read(judgingServiceProvider);
...
final eventScoringStatsProvider =
...
final submissionScoresStreamProvider =
    StreamProvider.family<List<Score>, String>((ref, submissionId) {
...
final scoringRubricStreamProvider =
    StreamProvider.family<ScoringRubric?, String>((ref, rubricId) {
  final rubricRepository = ref.read(scoringRubricRepositoryProvider);
  return rubricRepository.streamById(rubricId);
});

/// Provider for creating a new scoring rubric
final createScoringRubricProvider = Provider<
    Future<ScoringRubric> Function({
...
final cloneRubricProvider = Provider<
    Future<ScoringRubric> Function({
...
final submitScoreProvider = Provider<
...
final submissionCommentsProvider =
    FutureProvider.family<List<JudgeComment>, String>(
...
final submissionCommentsStreamProvider =
    StreamProvider.family<List<JudgeComment>, String>((ref, submissionId) {
  final judgingService = ref.read(judgingServiceProvider);
...
final eventJudgeAssignmentsProvider =
    FutureProvider.family<List<JudgeAssignment>, String>((ref, eventId) async {
  final judgingService = ref.read(judgingServiceProvider);
  return await judgingService.getEventJudges(eventId);
});

/// Stream provider for real-time judge assignments
final eventJudgeAssignmentsStreamProvider =
    StreamProvider.family<List<JudgeAssignment>, String>((ref, eventId) {
  final judgingService = ref.read(judgingServiceProvider);
  return judgingService.streamEventJudges(eventId);
});

/// Provider for getting judge assignments for a specific judge
final judgeAssignmentsProvider =
    FutureProvider.family<List<JudgeAssignment>, String>((ref, judgeId) async {
  final judgingService = ref.read(judgingServiceProvider);
  return await judgingService.getJudgeAssignments(judgeId);
});

/// Provider for checking if a judge is assigned to an event
final isJudgeAssignedProvider =
    FutureProvider.family<bool, (String, String)>((ref, params) async {
...
final judgePermissionProvider =
    FutureProvider.family<bool, (String, String, String)>((ref, params) async {
